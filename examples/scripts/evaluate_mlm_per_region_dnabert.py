...
def get_args():
    ...
    p.add_argument("-k", choices=(3, 4, 5, 6), type=int, required=True)
...
if __name__ == "__main__":
    ...
    ds = analysis_embedding.GenomicRegionData(
        dnabert_k=args.k,
        tokenizer=BertTokenizerFast.from_pretrained(f"/home/chenken/Documents/DNABERT/models/{args.k}-new-12w-0")
    )
    ...
    dnabert = AutoModelForMaskedLM.from_pretrained(f"/home/chenken/Documents/DNABERT/models/{args.k}-new-12w-0")
    ...
    with torch.no_grad(), autocast():
        for it, (ids, masked_ids, _, label, phastcons, phylop, annotation, repeat) in enumerate(tqdm(loader)):
            ...
            out = dnabert.forward(masked_ids)
            ...
            all_dnabert_logits.append(vert_logits[keep])
    ...
    all_dnabert_logits = torch.cat(all_dnabert_logits).float()
    ...
    print(all_label.shape, all_dnabert_logits.shape, file=sys.stderr)

    pickle.dump((all_label, all_dnabert_logits, all_anno, all_repeat), open(args.output, 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
    ...
    print("#region\tn\tacc_vert\tece_vert")
    print('\t'.join([
        "all", '.',
        "{:.3f}".format(acc_fun(all_label.numpy(), torch.argmax(all_dnabert_logits, dim=-1).numpy())),
        "{:.3f}".format(2**F.cross_entropy(all_dnabert_logits, all_label).item()),
    ]))
    print()
    ...
    for i in range(all_anno.shape[1]):
        ...
        print('\t'.join([
            ...
            "{:.3f}".format(acc_fun(all_label[k].numpy(), torch.argmax(all_dnabert_logits[k], dim=-1).numpy())),
            "{:.3f}".format(acc_fun(all_label[k_repeat].numpy(), torch.argmax(all_dnabert_logits[k_repeat], dim=-1).numpy())),
            "{:.3f}".format(acc_fun(all_label[k_nonrepeat].numpy(), torch.argmax(all_dnabert_logits[k_nonrepeat], dim=-1).numpy())),
        ]))
