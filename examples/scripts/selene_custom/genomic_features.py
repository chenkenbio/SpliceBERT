...
def _get_feature_data(chrom, start, end,
                      ...
                      nt_level=False, strand='.', binary: bool=False,
                      default: float=0):
    """
    ...
    if rows is None:
        raise ValueError("No rows found for region: {}:{}-{}".format(
            chrom, start, end))
    x = _fast_get_feature_data(
        start, end, thresholds, feature_index_dict, rows, nt_level, strand, binary, default_value=default)
    return x
...
class TabixFileDumpClass(object):
    def __init__(self) -> None:
        pass
    def query(self, chrom, start: int, end: int):
        pass
...
class GenomicFeatures(Target):
    ...
    Accepts a tabix-indexed BED4+ file.
    ...
    def __init__(self, input_path: str, features: List[str], nt_level: bool=True, binary: bool=False, feature_thresholds=None, init_unpicklable=False):
        """
        Constructs a new `GenomicFeatures` object.
        """
        self.input_path = input_path
        assert os.path.exists(self.input_path + ".tbi")
        assert nt_level in {True, False}
        assert binary in {True, False}
        self.nt_level = int(nt_level)
        self.binary = int(binary)
        if not self.binary:
            pass

        if isinstance(features, str):
            try:
                self.features = json.load(open(features))
            except json.JSONDecodeError:
                self.features = list()
                with open(features) as infile:
                    for l in infile:
                        self.features.append(l.strip())
        if isinstance(features, dict):
            self.features = self.features
        else:
            self.feature_index_dict = dict(
                [(feat, index) for index, feat in enumerate(self.features)])
        self.n_features = len(set(self.feature_index_dict.values()))

        # self.index_feature_dict = dict(list(enumerate(features)))
        self.index_feature_dict = dict()
        for name, idx in self.feature_index_dict.items():
            if idx not in self.index_feature_dict:
                self.index_feature_dict[idx] = name
            else:
                if isinstance(self.index_feature_dict[idx], str):
                    self.index_feature_dict[idx] = [self.index_feature_dict[idx]]
                self.index_feature_dict[idx].append(name)
    ...
    def _query_tabix(self, chrom, start, end) -> Union[List[List[str]], None]:
        """
    ...
    @init
    def get_feature_data(self, chrom, start, end, strand='.', default_value: float=0):
        """
        ...
        """
        strand = str(strand)
        if (not self.nt_level) and self._feature_thresholds_vec is None:
        ...
        x = _get_feature_data(
            ...
            strand=strand, binary=self.binary, default=default_value)
        return  x
